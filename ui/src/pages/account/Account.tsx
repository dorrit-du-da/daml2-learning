...
import { assetDepositColDef, colDef } from "./config";
...
const Account: React.FC = () => {
  ...
  const allAssetDeposits = userContext
    .useStreamQueries(AssetDeposit, () => [], [account])
    .contracts.filter((c) => c.payload.account.id.label === account);

  const depositsRowData: any[] = [];
  allAssetDeposits.forEach((i) => {
